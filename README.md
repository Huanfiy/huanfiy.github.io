...
- **工作室** — Three.js 手绘质感的嵌入式工作室（`studio.html`）：可拖动环视、缩放和切换分区镜头；包含 STM32 固件烧录与电路板展开、示波器波形 / 频率调节与精细化直流稳压电源模型、焊台与排烟联动、CoreXY 打印机逐层打印无人机机架、FOC 驱动的无刷电机台架调速和机械臂取放演示。新增绿植浇水与模拟土壤湿度联动。支持昼夜灯光、双扇开窗、设备导览和手机触控；无边框显示器直接承载可操作的 HTML huanfly-os，内含终端（始终是浏览器内的访客模拟）、相册与 Robot（仅保留「研究中」占位界面），靠近或退后不会切换桌面或中断应用。
- **个人展示** — 首页、关于页面，响应式布局
- **幻羽 OC 陪伴** — 森林右下角的轻量角色，点击切换六种姿态与离线互动，临时气泡说完即收起；保留星屿手记与六件本地收藏、默认关闭的语音适配接口
...
首页是一幅会随时间变化的手绘森林绘本：天空按本地时间在清晨、白昼、黄昏与夜晚之间切换，太阳沿弧线移动，夜里换成月牙、星星与草间萤火；远山、森林、山丘三层内联 SVG 随指针与滚动产生视差，落叶会被鼠标的风吹动。来自雾庭星屿的 OC「幻羽 / 小羽」缩小安放在首页森林右下角，不再占据正文分栏或显示常驻操作区：根据时段问候，点击切换姿态与离线短句，闲置时打盹；气泡仅在说话时出现，阅读后自动收起，临时气泡里的手记入口可查看设定、日常插画与相遇收藏。主题切换以按钮为圆心扩散揭示，卡片悬停带有跟随指针的柔光与纸片倾斜，首页、博客、工具、关于及工具子页的共享草地页脚统一由睡着的小羽陪伴。交互采用原生 HTML / CSS / JavaScript，角色插画为自托管 WebP，遵循 `prefers-reduced-motion`。
...
├── js/script.js          # 通用脚本
├── css/oc.css            # 首页幻羽与星屿手记样式
├── js/oc-character.js    # 独立角色交互、姿态加载、收藏与生命周期
├── js/oc-world.js        # OC 设定、离线语料与资产索引
├── js/oc-voice.js        # 可替换语音接口（默认无 provider）
├── picture/oc/           # 六种姿态与六件 SVG 收藏图标
├── js/hero-scene.js      # 首页 Hero 景观：时段天空、视差、落叶 / 萤火
├── js/studio.js          # 设备状态、镜头导航与电脑入口
├── js/studio-room.js     # Three.js 场景、程序化模型与设备动画
├── js/studio-instruments.js # 示波器 / 直流电源模型与共用面板布局
├── js/studio-instrument-art.js # 仪器静态图集、读数与波形绘制
...
测量仪器回归见 `tests/studio-instruments.html`：验证示波器与电源叠放、接口凹槽、波形切换 / 暂停、静态纹理及资源清理；也包含在下方可选 Chromium 回归中。
